...
class WhiteListRemoveUseCaseImplTest {

    private val mockDeviceManager: DeviceManager = mockk()
    private val mockPackageIdSelectedStateFlow = MutableStateFlow<String?>(null)
    private lateinit var useCase: WhiteListRemoveUseCaseImpl

    @Before
    fun setUp() {
        useCase = WhiteListRemoveUseCaseImpl(mockDeviceManager)
    ...
    fun `invoke() should emit success result with true when shell command succeed`() =
        runTest {
            ...
            coEvery { mockDeviceManager.executeShellCommand(any()) } coAnswers {
                if (args.first() is WhiteListAdbCommands.Remove) {
