...
class GetMobileDataStateUseCaseImplTest {

    private val mockDeviceManager: DeviceManager = mockk()
    private lateinit var useCase: GetMobileDataStateUseCaseImpl

    @Before
    fun setUp() {
        useCase = GetMobileDataStateUseCaseImpl(mockDeviceManager)
    }
    ...
    @Test
    fun `invoke() should emit success result when shell command succeeds and mobile data is enabled`() = runTest {
        ...
        coEvery { mockDeviceManager.executeShellCommand(any()) } coAnswers {
            if (args.first() is InternetAdbConnectionCommands.GetMobileDataState) {
    ...
    @Test
    fun `invoke() should emit success result when shell command succeeds and mobile data is disabled`() = runTest {
